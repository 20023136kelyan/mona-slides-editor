...
不监听任何端口。文档仍保存在用户选择的原始文件夹中；当用户要求代理处理文档时，
应用会通过原生 Claude 或 Codex 执行框架，把临时工作区交给所选模型。
...
代理通过 Claude Agent SDK 或 Codex app-server 运行在桌面外壳自己的进程中，
使用本机已有的 Claude 或 ChatGPT 订阅登录。Mona 不保存任何一方的凭据。
与提供商无关的对话记录让用户可以在两轮生成之间切换模型，同时保留已有上下文，
并继续使用同一套聊天界面。
...
会构建渲染进程、打包外壳，并将安装包输出到 `.artifacts/desktop/`。产物未签名：
签名和公证需要 Apple 开发者身份，因此 macOS 首次启动时会给出提示。

应用体积较大，因为两个原生代理执行框架都包含平台相关的可执行文件。它们会被解包到
asar 之外，因为子进程无法从 asar 内部启动。
