...
test('does not activate snapping or guides until a real drag begins', async ({ page }) => {
  const target = page.getByRole('button', { name: 'Select shape fixture-radial-shape' })
  const rendered = page.locator('.mona-render-stage [data-element-id="fixture-radial-shape"]')
...
test('supports grouped selection, create gestures, context settings, clipboard, and deletion', async ({ page }) => {
  ...
  const groupedShape = page.getByRole('button', { name: 'Select shape fixture-gradient-shape' })
  const groupedText = page.locator('.mona-editor-slide-canvas [data-element-id="fixture-title"] .mona-text-content')
  ...
  await expect(page.getByRole('combobox', { name: 'Link target' })).toHaveText('Slide 2')
  ...
  const groupPosition = () => page.evaluate(() => {
    ...
    return {
      shape: elements.find(element => element.id === 'fixture-gradient-shape')!.left,
      text: elements.find(element => element.id === 'fixture-title')!.left,
    }
  })
...
test('crops an image and supports lasso selection, zoom, and spacebar panning', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Select image fixture-image-round' }).click()
  const cropModelBefore = await page.evaluate(() => {
    const element = window.__MONA_TEST__!.getState().presentation.slides[1]!.elements
      .find(item => item.id === 'fixture-image-round')!
    return structuredClone(element)
  })
  // The inspector opens on demand from the context toolbar's deep actions.
  await page.locator('.mona-contextual-deep-actions').getByRole('button', { name: 'Style' }).click()
  ...
  const panBefore = await page.evaluate(() => window.__MONA_TEST__!.getState().session.canvasPan)
  await page.mouse.move(stageBox!.x + 70, stageBox!.y + 70)
  await page.mouse.down()
  await page.mouse.move(stageBox!.x + 120, stageBox!.y + 100, { steps: 2 })
  await page.mouse.up()
  await page.keyboard.up(' ')
  await expect.poll(() => page.evaluate(() => window.__MONA_TEST__!.getState().session.canvasPan)).toEqual({
    x: panBefore.x + 50,
    y: panBefore.y + 30,
  })
})
