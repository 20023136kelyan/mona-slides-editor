...
import { EditorPptistSelect } from '@/features/editor/EditorPptistSelect'
...
export function EditorMarkupPanel({ onClose, runtime }: { onClose: () => void; runtime: EditorRuntime }) {
  ...
  const updateSlide = (type: string) => runtime.commit('Label slide type', type
    ...
    : [{ type: 'slide.properties.remove', payload: { id: slide.id, property: 'type' } }], { recordHistory: false })
  const updateElement = (type: string) => {
    ...
    if (element.type === 'image') {
      runtime.commit('Label image type', type
        ...
        : [{ type: 'element.properties.remove', payload: { id: element.id, property: 'imageType' } }], { recordHistory: false })
    }
    else if (element.type === 'text') {
      runtime.commit('Label text type', type
        ? [{ type: 'element.update', payload: { id: element.id, props: { textType: type as TextType } } }]
        : [{ type: 'element.properties.remove', payload: { id: element.id, property: 'textType' } }], { recordHistory: false })
    }
      ...
      runtime.commit('Label shape text type', [{ type: 'element.update', payload: { id: element.id, props: { text } } }], { recordHistory: false })
    }
  }
  ...
  return (
    <EditorMoveablePanel className="mona-markup-panel" height={130} left={-270} onClose={onClose} title={t('foundation.editor.markup.title')} top={90} width={300}>
      <div className="mona-markup-container">
        <div className="mona-markup-row">
          <div>{t('foundation.editor.markup.currentSlide')}</div>
          <EditorPptistSelect ariaLabel={t('foundation.editor.markup.currentSlide')} onChange={updateSlide} options={slideTypeOptions} value={slide.type ?? ''} />
        </div>
        {semanticKind ? (
          <div className="mona-markup-row">
            <div>{t(`foundation.editor.markup.${semanticKind === 'image' ? 'currentImage' : 'currentText'}`)}</div>
            <EditorPptistSelect ariaLabel={t(`foundation.editor.markup.${semanticKind === 'image' ? 'currentImage' : 'currentText'}`)} onChange={updateElement} options={semanticOptions} value={semanticValue} />
          </div>
