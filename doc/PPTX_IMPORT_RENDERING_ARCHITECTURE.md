...
| B09 | Headers, footers, date, and slide numbers | Partial; reserved fields and native master policy are supported | Master policy and layout field placeholders render in inherited positions; `firstSlideNum`, locale-aware reserved date/time formats, semantic per-slide field creation/editing, and explicit master-policy writeback are supported. Unknown application-defined field types retain their cached text. Remaining breadth is generator-specific custom field behavior and exhaustive cross-author validation of inherited placement | Dynamic/static footer fields display in the correct inherited position |
...
| D06 | Autofit and measurement | Partial | Imported shrink factors and line-spacing reduction render consistently. Dynamic no-fit/shrink/resize measurement already runs in the editor, thumbnails, and read-only renderer and invalidates when fonts load or substitute. Remaining work is Office-compatible deterministic font metrics, exact line breaking, horizontal/no-wrap overflow, shape text rectangles, and vertical/rotated clipping | Text fits the same bounds at editor, thumbnail, and read-only scales |
...
| D09 | Fields and links | Done for reserved fields and run links; partial action breadth | Fields and run-level hyperlinks remain typed. Slide numbers honor presentation order and `firstSlideNum`; `datetime`, `datetime1`–`datetime13`, and `datetimeFigureOut` evaluate with run language and a deterministic render instant. Structured JSON can create, retarget, materialize, or remove native `a:fld` runs without duplicating an HTML edit. External/internal run links and relationship-free slideshow actions write back; unknown custom field types retain cached text. Remaining work is element/hover actions and sounds | Field semantics remain distinct from their current displayed string |
