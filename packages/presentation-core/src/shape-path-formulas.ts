import { ShapePathFormulasKeys } from './model'

const shapeValue = (values: number[] | undefined, index: number) => values?.[index] ?? 0
...
export const SHAPE_PATH_FORMULAS: Record<string, ShapePathFormula> = {
  [ShapePathFormulasKeys.ROUND_RECT]: {
    ...
    formula: (width, height, values) => {
      const radius = Math.min(width, height) * shapeValue(values, 0)
  ...
  [ShapePathFormulasKeys.MESSAGE]: {
    ...
    formula: (width, height, values) => {
      const point = width * shapeValue(values, 0)
      const arrowWidth = width * 0.2
      const arrowheight = height * shapeValue(values, 1)
  ...
  [ShapePathFormulasKeys.L]: {
    ...
    getBaseSize: [(width, height) => Math.min(width, height)],
    formula: (width, height, values) => {
      const lineWidth = Math.min(width, height) * shapeValue(values, 0)
  ...
  [ShapePathFormulasKeys.DIAGSTRIPE]: {
    ...
    formula: (width, height, values) => {
      const point = Math.min(width, height) * shapeValue(values, 0)
      if (width >= height) {
  ...
  [ShapePathFormulasKeys.TRIANGLE]: {
    ...
    getBaseSize: [width => width],
    formula: (width, height, values) => {
      const vertex = width * shapeValue(values, 0)
  ...
  [ShapePathFormulasKeys.BULLET]: {
    ...
    formula: (width, height, values) => {
      const point = height * shapeValue(values, 0)
