...
try {
  for (let index = 0; index < sampleCount; index++) {
    ...
    if (readySelector) {
      ...
      await page.locator('.mona-editor-stage[role="application"]').waitFor({ state: 'visible' })
      await page.locator('.mona-editor-slide-canvas').waitFor({ state: 'visible' })
      await page.locator('.mona-thumbnail-item').first().waitFor({ state: 'visible' })
    }
