<template>
  <div class="link-handler" :style="handlerStyle">
    ...
    <a class="link" v-else-if="elementInfo.link" @click="turnTarget(elementInfo.link.target)">{{ $t('canvas.slideLink', { number: elementInfo.link.target }) }}</a>
    <div class="btns">
      <div class="btn" @click="openLinkDialog()">{{ $t('canvas.change') }}</div>
      <Divider type="vertical" />
      <div class="btn" @click="removeLink(elementInfo)">{{ $t('canvas.remove') }}</div>
    </div>
  </div>
</template>
