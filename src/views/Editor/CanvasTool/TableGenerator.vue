<template>
  <div class="table-generator">
    <div class="title">
      <div class="lef">{{ endCell.length ? $t('table.dimensions', { rows: endCell[0], columns: endCell[1] }) : $t('editor.table') }}</div>
      <div class="right" @click="isCustom = !isCustom">{{ isCustom ? $t('table.back') : $t('common.custom') }}</div>
    </div>
    <table 
      @mouseleave="endCell = []" 
    ...
    <div class="custom" v-else>
      <div class="row">
        <div class="label" style="width: 25%;">{{ $t('table.rows') }}</div>
        <NumberInput
          :min="1"
          :max="20"
      ...
      <div class="row">
        <div class="label" style="width: 25%;">{{ $t('table.columns') }}</div>
        <NumberInput
          :min="1"
          :max="20"
...
const insertCustomTable = () => {
  if (customRow.value < 1 || customRow.value > 20) return message.warning(t('table.dimensionsError'))
  if (customCol.value < 1 || customCol.value > 20) return message.warning(t('table.dimensionsError'))
