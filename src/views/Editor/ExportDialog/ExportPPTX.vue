  ...
  <div class="export-pptx-dialog">
    ...
    <div class="configs">
      <div class="row">
        ...
        >
          <RadioButton style="width: 33.33%;" value="all">{{ $t('common.all') }}</RadioButton>
          <RadioButton style="width: 33.33%;" value="current">{{ $t('common.currentSlide') }}</RadioButton>
          <RadioButton style="width: 33.33%;" value="custom">{{ $t('common.custom') }}</RadioButton>
        </RadioGroup>
      </div>
      <div class="row">
        ...
        >
          ...
          <RadioButton style="width: 50%;" value="image">{{ $t('export.imageOnly') }}</RadioButton>
        </RadioGroup>
      </div>
      <div class="row" style="margin-bottom: 32px" v-if="rangeType === 'custom'">
        <div class="title" :data-range="`(${range[0]}–${range[1]})`">{{ $t('common.customRange') }}:</div>
        <Slider
          class="config-item"
          range
          :min="1"
      ...
      <template v-if="exportMode === 'standard'">
        <div class="row">
          ...
          <div class="config-item">
            <Switch v-model:value="ignoreMedia" v-tooltip="$t('export.ignoreMediaHelp')" />
          </div>
        </div>
        <div class="row">
          <div class="title">{{ $t('export.overwriteMaster') }}</div>
          <div class="config-item">
    ...
    <div class="btns">
      <Button class="btn export" type="primary" @click="execExport()"><i-icon-park-outline:download /> {{ $t('export.exportPptx') }}</Button>
      <Button class="btn close" @click="emit('close')">{{ $t('common.close') }}</Button>
    </div>

    <FullscreenSpin :loading="exporting" :tip="$t('common.exporting')" />
  </div>
</template>
