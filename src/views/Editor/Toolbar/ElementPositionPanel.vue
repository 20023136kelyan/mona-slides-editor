<template>
  <div class="element-positopn-panel">
    <div class="title">{{ $t('toolbar.layer') }}</div>
    <ButtonGroup class="row">
      <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.TOP)"><i-icon-park-outline:send-to-back /> {{ $t('toolbar.bringFront') }}</Button>
      <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.BOTTOM)"><i-icon-park-outline:bring-to-front-one /> {{ $t('toolbar.sendBack') }}</Button>
    </ButtonGroup>
    <ButtonGroup class="row">
      <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.UP)"><i-icon-park-outline:BringToFront /> {{ $t('toolbar.moveForward') }}</Button>
      <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.DOWN)"><i-icon-park-outline:SentToBack /> {{ $t('toolbar.moveBackward') }}</Button>
    </ButtonGroup>

    <Divider />
    
    <div class="title">{{ $t('toolbar.alignment') }}</div>
    <ButtonGroup class="row">
      <Button style="flex: 1;" v-tooltip="$t('common.leftAlign')" @click="alignElementToCanvas(ElementAlignCommands.LEFT)"><i-icon-park-outline:align-left /></Button>
      <Button style="flex: 1;" v-tooltip="$t('common.horizontalCenter')" @click="alignElementToCanvas(ElementAlignCommands.HORIZONTAL)"><i-icon-park-outline:align-vertically /></Button>
      <Button style="flex: 1;" v-tooltip="$t('common.rightAlign')" @click="alignElementToCanvas(ElementAlignCommands.RIGHT)"><i-icon-park-outline:align-right /></Button>
    </ButtonGroup>
    <ButtonGroup class="row">
      <Button style="flex: 1;" v-tooltip="$t('common.topAlign')" @click="alignElementToCanvas(ElementAlignCommands.TOP)"><i-icon-park-outline:align-top /></Button>
      <Button style="flex: 1;" v-tooltip="$t('common.verticalCenter')" @click="alignElementToCanvas(ElementAlignCommands.VERTICAL)"><i-icon-park-outline:align-horizontally /></Button>
      <Button style="flex: 1;" v-tooltip="$t('common.bottomAlign')" @click="alignElementToCanvas(ElementAlignCommands.BOTTOM)"><i-icon-park-outline:align-bottom /></Button>
    </ButtonGroup>
    ...
    <template v-if="handleElement!.type !== 'line'">
      <div class="row">
        ...
        <template v-if="['image', 'shape', 'audio'].includes(handleElement!.type)">
          <span style="width: 10%;" class="icon-btn" :class="{ 'active': fixedRatio }" v-tooltip="fixedRatio ? $t('toolbar.unlockAspectRatio') : $t('toolbar.lockAspectRatio')" @click="updateFixedRatio(!fixedRatio)">
