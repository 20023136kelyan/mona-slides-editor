<template>
  ...
  >
    <div class="video-wrap" @click="toggle()">
      <div class="load-error" v-if="loadError">{{ $t('player.loadFailed') }}</div>
      ...
      <div class="icons icons-right">
        <div class="speed">
          <div class="icon speed-icon">
            <span class="icon-content" @click="speedMenuVisible = !speedMenuVisible">{{ playbackRate === 1 ? $t('player.speed') : (playbackRate + 'x') }}</span>
        ...
        <div class="loop" @click="toggleLoop()">
          <div class="icon loop-icon" :class="{ 'active': loop }">
            <span class="icon-content">{{ $t('player.loopStatus', { status: $t(loop ? 'player.on' : 'player.off') }) }}</span>
          </div>
        </div>
      </div>
