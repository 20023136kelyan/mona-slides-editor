...
declare global {
  interface Window {
    __MONA_STABILITY_DIAGNOSTICS__?: {
      clearLongTasks: () => void
...
async function installDiagnostics(context: BrowserContext) {
  await context.addInitScript(() => {
    ...
    Object.defineProperty(window, '__MONA_STABILITY_DIAGNOSTICS__', {
      configurable: false,
      value: {
...
async function openMona(page: Page) {
  ...
  await expect(page.locator('.mona-thumbnail-item').first()).toBeVisible()
...
async function cycleTransientSurfaces(page: Page) {
  ...
  // Task panels carry no visible title or close button; the drawer is named
  // by its aria-label and closed by the border pill.
  const commentsPanel = page.getByRole('complementary', { name: /Comments on slide/ })
...
// The editor stays mounted-but-hidden (<Activity>) while presenting, so
// screening round trips are part of the leak/retention budget: hidden-tree
// effects must fully detach and reattach without accumulating listeners or
// heap (doc/QUIRK_RETIREMENT.md).
async function cycleScreening(page: Page) {
...
test('production listeners and retained heap stabilize across repeated mount cycles', async ({ browser }) => {
  ...
  for (let index = 0; index < 3; index += 1) {
    await cycleTransientSurfaces(page)
    await cycleScreening(page)
  }
  const listenersBefore = await page.evaluate(() => window.__MONA_STABILITY_DIAGNOSTICS__!.listenerSnapshot())
  ...
  const listenersAfter = await page.evaluate(() => window.__MONA_STABILITY_DIAGNOSTICS__!.listenerSnapshot())
  ...
  console.log(`[production:stability] listeners ${Object.values(listenersBefore).reduce((sum, count) => sum + count, 0)} -> ${Object.values(listenersAfter).reduce((sum, count) => sum + count, 0)}; heap ${heapBefore} -> ${heapAfter} bytes`)
  await context.close()
})
...
test('production navigation stays within a frame budget without long tasks or DOM replacement', async ({ browser }) => {
  ...
  await firstThumbnail.evaluate(element => element.setAttribute('data-stability-node', 'true'))
  await page.evaluate(() => window.__MONA_STABILITY_DIAGNOSTICS__!.clearLongTasks())
  ...
  const longTasks = await page.evaluate(() => window.__MONA_STABILITY_DIAGNOSTICS__!.longTasks())
  expect(p95).toBeLessThanOrEqual(40)
  expect(longTasks).toEqual([])
  expect(await firstThumbnail.getAttribute('data-stability-node')).toBe('true')
  console.log(`[production:stability] navigation p95 ${p95.toFixed(2)}ms; long tasks ${longTasks.length}; thumbnail DOM identity preserved`)
  await context.close()
})
